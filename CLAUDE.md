...
### Supabase（適合：班級教學 + 需要分析）
- ✅ 班級成績、學期累積資料
- ✅ 學生作業、評量紀錄
- ✅ 老師用 Claude + MCP 直接查詢分析（`execute_sql`）
- ✅ 教學駕駛艙、差異化派題（EP10）
- ⚠️ **限制**：免費版並發連線上限 200 → 不適合大型研習

### Firebase（適合：大型即時互動）
- ✅ 千人研習 IRS（並發連線上限 100 萬）
- ✅ 即時文字雲、投票、舉手
- ✅ `onSnapshot` 即時更新更簡單
- ✅ 不會閒置暫停
- ⚠️ **限制**：MCP 無法直接查 Firestore 資料，老師不能用 Claude 直接問成績

### 場景對照表

| 場景 | 規模 | 推薦 | 原因 |
|------|------|------|------|
| 班級成績記錄本 | 30 人 | Supabase | 老師要查資料 |
| 教學駕駛艙 + 差異化派題 | 30 人 | Supabase | 第六段需要 MCP 查成績 |
| 課堂 IRS（小班） | 30 人 | 都行 | Firebase 寫起來稍簡單 |
| **大型研習 IRS** | **1000 人** | **Firebase** | **Supabase 免費並發 200 不夠** |
| 即時文字雲、投票牆 | 不限 | Firebase | onSnapshot 即時更新 |
| 學期累積分析 | — | Supabase | SQL 統計查詢 |
